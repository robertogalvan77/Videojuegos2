...
trait ContextProvider extends FlatSpec with BeforeAndAfterAll with Matchers {
  ...
  val config: Config = new ProcessConfigLoader().fromPath("src/test/resources/config/AplicationTest.conf")
  val configJoinException: Config = new ProcessConfigLoader().fromPath("src/test/resources/config/AplicationJoinExceptionTest.conf")
